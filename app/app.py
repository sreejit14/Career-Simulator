...
@st.cache_resource
def get_bundle():
    return be.load_bundle("salary_model.pkl")
...
col1.metric("Current salary",
            f"${be.predict_salary(bundle, base_row):,.0f}")
col2.metric(f"Salary after {years} yr",
            f"${scenario_df.iloc[-1]['Predicted_Salary']:,.0f}",
            f"{scenario_df.iloc[-1]['Predicted_Salary'] - baseline_df.iloc[-1]['Predicted_Salary']:,.0f}")

fig = px.line(scenario_df, x="Year", y="Predicted_Salary",
              title="Salary trajectory (scenario)")
fig.add_scatter(x=baseline_df["Year"], y=baseline_df["Predicted_Salary"],
